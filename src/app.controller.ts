  ...
  async renderTemplate(@Body() raw): Promise<string> {
    if (typeof raw === 'string' || Buffer.isBuffer(raw)) {
      ...
      const allowedTemplates = {
        'template1': 'Hello, {{=it.name}}!',
        'template2': 'Welcome to {{=it.place}}!'
      };

      if (!allowedTemplates[text]) {
        throw new HttpException('Template not allowed', HttpStatus.BAD_REQUEST);
      }

      const compiled = dotT.template(allowedTemplates[text]);
      const res = compiled({ name: 'User', place: 'our platform' });
